...
The solvers above are harnesses you assemble yourself with ellmer. To see how an off-the-shelf coding agent stacks up on the same problems, vitals provides the solvers `claude_code()` and `codex()`, which evaluate the Claude Code and Codex agents on a task's dataset:

```{r}
#| label: are-claude-code
#| eval: false
...
These solvers run the agent's command line interface in a Docker sandbox via Python Inspect (resolved automatically with reticulate—you'll need the reticulate package installed and a running Docker daemon). The agent's model calls are proxied, so your API key never enters the sandbox, and its transcript is read back into ellmer Chat objects: tool calls appear in the log viewer, and token usage is tallied as usual.

Since these are ordinary solvers, the same R scorer grades both your harness and the agent, and results compare directly:

```{r}
#| label: bind-claude-code
#| eval: false
